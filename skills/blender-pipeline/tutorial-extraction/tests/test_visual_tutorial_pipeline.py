...
def fixture(root: Path):
    ...
    for number, color in [(0, "white"), (4, "red"), (9, "blue"), (14, "green")]:
        ...
        frames[name] = {"path": name, "timestamp": number,
                        "sha256": visual.transport.sha256_path(path)}
    doc = {"content_name": "几何练习", "purpose": "制作三个有序网格步骤。",
        "starting_scene": "默认场景", "closing": "保留模型结构并另存工程。",
        "final_result_description": "三个依次完成的网格操作。", "learner_inputs": [],
        "missing_inputs": [], "cover_frame": "frames/frame_000000000.jpg", "steps": []}
    for i, t in enumerate([4, 9, 14], 1):
        frame = f"frames/frame_{t * 1000:09d}.jpg"
        doc["steps"].append({"number": i, "title": f"完成第{i}个网格操作",
            "time_start": t-3, "time_end": t, "actions": [f"选择网格并执行操作{i}"],
            "expected": f"网格状态{i}", "caption": f"操作{i}后的网格",
            "evidence_frame": frame, "claims": [{"text": f"执行操作{i}",
                    "status": "shown", "at": t, "frame": frame}]})
    source = {"id": "fixture", "title": "几何练习", "duration_seconds": 15,
              "url": "https://example.com/tutorial"}
    rubric = {"schema_version": "1.0", "total_points": 100,
        "status_weights": {"PASS": 1.0, "PARTIAL": .5, "FAIL": .0},
        "capability_totals": {"GEO": 100}, "rubric": [{"id": "GEO-01", "capability": "GEO",
            "points": 100, "criterion": "网格存在", "scoring_rule": "存在得100分，否则0分",
            "artifact_verifier_check": "GEO-01"}], "artifact_verifier": {"engine": "blender_python",
...
class VisualTutorialTests(unittest.TestCase):
    def test_package_and_adapter_preserve_whole_procedure_without_rubric_leak(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            files = visual.write_package(package, doc, rubric, source, analysis, frames, {})
            self.assertEqual([], list((package / "input").iterdir()))
            self.assertEqual(3, len(check.content_entries(package / "output")))
            visual.publish_workspace(workspace, package, doc, source, files, {}, frames, True)
            ...
            self.assertEqual(3, len(json.loads((workspace / "steps_verified.json").read_text())["steps"]))
            ...
            try:
                ...
                self.assertEqual(3, len(run_video_strict_replay.ordered_tutorial_visual_evidence(workspace)))
                self.assertEqual("pass", run_video_replay_main.tutorial_visual_text_status(workspace)["status"])
            finally:
                sys.path.pop(0)
    ...
    def test_actual_inputs_are_copied_and_paths_rebased(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            workspace = root / "run"
            package = workspace / "tutorial_package" / doc["content_name"]
            files = visual.write_package(package, doc, rubric, source, root / "analysis", frames, {asset.name: asset})
            visual.publish_workspace(workspace, package, doc, source, files, {}, frames)
            self.assertEqual(asset.read_bytes(), (package / "input" / asset.name).read_bytes())
            self.assertIn("tutorial_package/几何练习/input/original.blend", (workspace / "tutorial.md").read_text())
    ...
    def test_cli_calls_new_skill_with_existing_arguments(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            with patch.object(cli, "extract_visual_tutorial", return_value={"status": "complete"}) as extract:
                self.assertEqual(0, cli.main(["--video-file", str(video), "--title", "Test",
                    "--output-dir", str(root / "out"), "--provider", "codex-cli", "--workspace-mode",
                    "--window-budget", "0"]))
    ...
    def test_new_extraction_orchestration_uses_skill_not_legacy_fragments(self):
        with tempfile.TemporaryDirectory() as raw:
            ...
            ledger = {"summary": "完整几何流程", "steps": doc["steps"],
                      "evidence_requests": [], "final_result_candidates": [doc["cover_frame"]]}
            ...
            with patch.object(visual.prepare_video, "prepare", side_effect=prepare), \
                 patch.object(visual.transport, "ffprobe_duration", return_value=15), \
                 patch.object(visual.transport, "resolve_transcript", return_value=
                              visual.transport.TranscriptResult("unavailable", "none", warning="No transcript supplied")), \
                 patch.object(visual.transport, "CodexCliModelClient", Client), \
                 patch.object(visual.transport, "extract_tutorial") as old_extractor:
                manifest = visual.extract_visual_tutorial(video_file=video, video_url=None,
                    title="完整几何流程", output_dir=root / "out", profile_name="balanced",
                    model="gpt-5.6-sol", transcript=None, render_html_enabled=False,
                    provider="codex-cli", workspace_mode=True, max_calls=5,
                    cache_dir=root / "cache")
                rerun = visual.extract_visual_tutorial(video_file=video, video_url=None,
                    title="完整几何流程", output_dir=root / "out", profile_name="balanced",
                    model="gpt-5.6-sol", transcript=None, render_html_enabled=False,
                    provider="codex-cli", workspace_mode=True, max_calls=5,
                    cache_dir=root / "cache", replace_existing=True)
