...
def instructions() -> str:
    paths = [ROOT / "SKILL.md", *[ROOT / "references" / name for name in
             ("evidence-ledger.md", "markdown-layout.md", "blender-capability-rubric.md")]]
...
def sampling(duration: float, profile: str) -> float:
    ...
    return base * {"economy": 2.0, "balanced": 1.0, "forensic": .5}[profile]


class CachedCalls:
    def __init__(self, client: Any, root: Path, context: str, key: str = "", endpoint: str = ""):
    ...
    def call(self, stage: str, prompt: str, images: Sequence[Path]) -> dict:
        prompt = self.context + "\n\n" + prompt
        identity = transport.canonical_sha256({"model": self.client.model,
                    "provider": type(self.client).__name__, "prompt": prompt,
...
def validate_document(doc: Mapping[str, Any], frames: Mapping[str, Mapping[str, Any]],
                      duration: float, available_inputs: set[str]) -> None:
    ...
    if doc.get("missing_inputs"):
        raise Error("required learner assets are unavailable: " + "; ".join(map(str, doc["missing_inputs"])))
    inputs = doc.get("learner_inputs", [])
    if not isinstance(inputs, list):
        raise Error("learner_inputs must be a list")
    for item in inputs:
        if item.get("name") not in available_inputs or not str(item.get("use", "")).strip():
    ...
    for i, step in enumerate(steps, 1):
        ...
        if not isinstance(step.get("actions"), list) or not step["actions"] or any(
                not isinstance(action, str) or not action.strip() for action in step["actions"]):
...
def render_markdown(doc: Mapping[str, Any], source: Mapping[str, Any]) -> str:
    lines = [f"# {doc['content_name']}教程", "", doc["purpose"], "",
             f"![视频中的最终效果](image/{Path(doc['cover_frame']).name})", "",
             doc["final_result_description"], "", "## You will need", "",
             "目标版本：Blender 4.1 / 5.1.2。" + str(doc.get("compatibility_notes") or
             "以下步骤按视频界面描述，未执行跨版本复现验证。"), "",
    ...
    if parameters:
        ...
        for row in parameters:
            lines.append("| " + str(row["setting"]).replace("|", "\\|") + " | " +
                         str(row["value"]).replace("|", "\\|") + " |")
        lines.append("")
    for step in doc["steps"]:
        lines.extend([f"## {step['number']}. {step['title']}（{timestamp(step['time_start'])}–{timestamp(step['time_end'])}）", ""])
        lines.extend(f"{i}. {action}" for i, action in enumerate(step["actions"], 1))
        lines.extend(["", f"![{step['title']}](image/{Path(step['evidence_frame']).name})",
                      "", step["caption"], ""])
    ...
    lines.extend(["## 收尾与保存", "", doc["closing"], "",
                  f"来源：[{source.get('id') or source.get('title')}]({source.get('url')})" if source.get("url")
                  else f"来源：{source.get('title')}（本地视频）", ""])
    return "\n".join(lines)


def write_package(package: Path, doc: Mapping[str, Any], rubric: Mapping[str, Any],
                  source: Mapping[str, Any], analysis: Path, frames: Mapping[str, Any],
                  assets: Mapping[str, Path]) -> dict:
    ...
    package_check.validate_markdown(tutorial, output / "image", allowed, analysis / doc["cover_frame"])
    try:
        package_check.validate_rubric(rubric_path)
    except SystemExit as exc:
        raise RubricError(str(exc)) from exc
    return {"tutorial": str(tutorial.relative_to(package)), "rubric": str(rubric_path.relative_to(package))}
...
def render_html(markdown_path: Path, destination: Path) -> None:
    ...
    transport.atomic_text(destination,
...
def publish_workspace(workspace: Path, package: Path, doc: Mapping[str, Any],
                      source: Mapping[str, Any], files: Mapping[str, str],
                      metadata: Mapping[str, Any], frames: Mapping[str, Any],
                      html_enabled: bool = False) -> dict:
    ...
    rebased = IMAGE_RE.sub(lambda m: m.group(0).replace(m.group(1), f"{prefix}/{m.group(1)}"), markdown)
    # Input paths in the learner package are relative to its subject directory.
    for item in doc.get("learner_inputs", []):
        rebased = rebased.replace(f"`input/{item['name']}`",
    ...
    for index, item in enumerate(doc["steps"]):
        ...
        steps.append({"step_id": f"STEP-{index + 1:03d}", "window_index": index,
            "time_range": f"{item['time_start']:.3f}-{item['time_end']:.3f}",
            "action": "；".join(item["actions"]), "object": str(item.get("object") or ""),
            "parameters": item.get("parameters", {}), "visual_result": item["expected"],
            ...
            "evidence": image_path, "source_claims": item["claims"]})
        windows.append({"index": index, "start_sec": item["time_start"],
            "end_sec": item["time_end"], "sheet": replay_image.relative_to(workspace).as_posix(),
            "frames": [{"path": replay_image.relative_to(workspace).as_posix(),
                        "time": frames[item["evidence_frame"]]["timestamp"]}]})
    transport.atomic_json(workspace / "steps_verified.json", {"schema": SCHEMA + ".steps",
        "authority": "structured projection of the visual tutorial; not legacy Claim Q-Gate", "steps": steps})
    transport.atomic_json(workspace / "tutorial_visual_contract.json", transport.tutorial_visual_contract(steps))
    transport.atomic_json(workspace / "rich_evidence" / "windows.json", windows)
    manifest = {"schema": SCHEMA, "skill": "video-to-visual-tutorial", "status": "complete",
        "source": dict(source), "title": doc["content_name"],
        "package": package.relative_to(workspace).as_posix(), **dict(metadata),
        ...
        "package_hashes": {p.relative_to(package).as_posix(): transport.sha256_path(p)
...
def validate_workspace(workspace: Path) -> list[str]:
    try:
        manifest = json.loads((workspace / "tutorial_manifest.json").read_text(encoding="utf-8"))
        ...
        if transport.sha256_path(workspace / "tutorial.md") != manifest["tutorial_sha256"]:
            return ["tutorial.md changed after package projection"]
        if transport.sha256_path(workspace / "steps_verified.json") != manifest["steps_sha256"]:
            return ["structured steps changed after package projection"]
        for relative, digest in manifest["package_hashes"].items():
            path = (package / relative).resolve()
            if not path.is_relative_to(package) or transport.sha256_path(path) != digest:
                return ["learner package changed after projection"]
        if (workspace / "tutorial_path_refs.md").read_bytes() != (workspace / "tutorial.md").read_bytes():
            return ["tutorial views differ"]
        for ref in IMAGE_RE.findall((workspace / "tutorial.md").read_text(encoding="utf-8")):
...
def extract_visual_tutorial(*, video_file: Path | None, video_url: str | None, title: str,
        ...
        provided_transcript_only: bool = False, asr_language_hint: str | None = None,
        provider: str = "api", fallback_reason: str = "", workspace_mode: bool = False,
        input_assets: Sequence[Path] = (), cache_dir: Path | None = None,
        max_calls: int | None = None, replace_existing: bool = False) -> dict:
    ...
    if replace_existing:
        if not workspace_mode:
            raise Error("--replace-existing is only supported in pipeline workspace mode")
        if old_manifest.is_file():
            ...
            if previous.get("schema") == SCHEMA:
                ...
                if owned_package.parent != workspace / "tutorial_package":
                    raise Error("previous manifest does not own a tutorial-package subject directory")
    cache_root = (cache_dir or workspace.parent / ".video-tutorial-cache").expanduser().resolve()
    ...
    for raw_asset in input_assets:
        ...
        if asset.name in assets or asset.suffix.lower() in {".mp4", ".mov", ".mkv", ".webm"}:
            raise Error("input assets must have unique names and must not include source video")
        assets[asset.name] = asset
    profile = replace(transport.PROFILES[profile_name], image_max_side=2400)
    with local_temporaries(cache_root), tempfile.TemporaryDirectory(prefix="source-", dir=cache_root) as temporary:
        source_temp = Path(temporary)
        platform = []
        if video_file:
            video = video_file.expanduser().resolve(strict=True)
            if source_url and not provided_transcript_only:
                platform = transport.fetch_platform_subtitles(source_url, source_temp / "subtitles")
        else:
            source_url = str(video_url)
            video, platform = transport.materialize_url(source_url, source_temp / "download")
        digest = transport.sha256_path(video)
        duration = transport.ffprobe_duration(video)
        cache = cache_root / (digest[:20] + "-" + profile_name)
        cache.mkdir(exist_ok=True)
        video_index = prepare_video.prepare(video, cache, sampling(duration, profile_name))
        frames = {r["path"]: r for r in video_index["frames"]}
        subtitle_cache = cache / "transcript.json"
        transcript_key = transport.canonical_sha256({"provided": transport.sha256_path(transcript) if transcript else "",
            "language": asr_language_hint, "provided_only": provided_transcript_only,
            "platform": [transport.sha256_path(p) for p in platform]})
        stored_transcript = json.loads(subtitle_cache.read_text()) if subtitle_cache.exists() else {}
        if stored_transcript.get("key") == transcript_key:
            speech = transport.TranscriptResult(**stored_transcript["value"])
        else:
            speech = transport.resolve_transcript(video, transcript, platform,
                platform_attempted=bool(source_url), allow_local_asr=not provided_transcript_only,
                asr_language_hint=asr_language_hint)
            transport.atomic_json(subtitle_cache, {"key": transcript_key, "value": asdict(speech)})
        budget = max_calls or profile.calls_per_ten_minutes * max(1, math.ceil(duration / 600))
        ...
        if budget < 5:
            raise Error("visual tutorial needs at least five calls including one repair reserve")
        key = transport.read_secret(secret_file) if provider == "api" and secret_file else ""
        if provider == "api":
            client = transport.ModelClient(endpoint=endpoint, key=key, model=model, profile=profile,
                                           call_budget=budget, max_output_tokens=18000)
        else:
            client = transport.CodexCliModelClient(model=model, profile=profile, call_budget=budget)
        caller = CachedCalls(client, cache, instructions(), key, endpoint)
        source = {"id": (re.search(r"BV[0-9A-Za-z]{10}", source_url) or [video.stem])[0],
                  "url": source_url, "title": title, "duration_seconds": duration, "sha256": digest}
        window_ledgers = []
        ...
        for number in range(chunks):
            ...
            for offset in range(0, len(rows), 24):
                ...
                prepare_video.contact_sheet([cache / r["path"] for r in rows[offset:offset+24]], sheet, cols=4, thumb_width=360)
                sheets.append(sheet)
            context = {"source": source, "window": [start, end], "frames": rows,
                "available_learner_assets": list(assets),
                "transcript": transport.segments_text(speech.segments, start, end, limit=16000),
                "previous_window_summary": window_ledgers[-1].get("summary", "") if window_ledgers else ""}
            coarse = caller.call(f"outline-{number}",
                ...
                json.dumps(context, ensure_ascii=False), sheets)
            detail_paths = []
            requests = coarse.get("evidence_requests", [])
            if not isinstance(requests, list) or len(requests) > max_requests:
                raise Error("model requested more focused frames than the selected profile permits")
            for request in requests:
                t = round(float(request["timestamp"]), 3)
                if not start <= t < min(duration, end + .001):
            ...
            for candidate in coarse.get("final_result_candidates", []):
                frame_name = candidate.get("frame") if isinstance(candidate, dict) else candidate
                if frame_name in frames:
                    detail_paths.append(cache / frame_name)
            detail_paths = list(dict.fromkeys(detail_paths))
            if not detail_paths:
                detail_paths = [cache / rows[-1]["path"]]
            inspected_frames.update(p.relative_to(cache).as_posix() for p in detail_paths)
            reviewed = caller.call(f"review-{number}",
                ...
                "context": context, "draft": coarse,
                    "attached_frames": [frames[p.relative_to(cache).as_posix()] for p in detail_paths]}, ensure_ascii=False), detail_paths)
            window_ledgers.append(reviewed)
        authoritative_frames = {k:v for k,v in frames.items() if k in inspected_frames}
...
"""
        document_context = json.dumps({"source": source, "ledgers": window_ledgers,
            "inspected_frames": list(authoritative_frames.values()), "available_inputs": list(assets)}, ensure_ascii=False)
        ...
        try:
            ...
            if doc.get("missing_inputs"):
                raise
            doc = caller.call("document-repair", prompt + document_context +
                "\nCorrect only these format/source-binding problems without inventing evidence: " + str(exc) +
                "\nDraft: " + json.dumps(doc, ensure_ascii=False), document_image)
            repair_used = True
            validate_document(doc, authoritative_frames, duration, set(assets))
...
"""
        rubric = caller.call("rubric", rubric_prompt + render_markdown(doc, source),
        ...
        with tempfile.TemporaryDirectory(prefix="package-", dir=cache_root) as staging:
            ...
            try:
                files = write_package(package, doc, rubric, source, cache, authoritative_frames, assets)
            except RubricError as exc:
                if repair_used:
                    raise Error("the single repair allowance was already used; " + str(exc)) from exc
                # One shared document/rubric repair allowance, never an unlimited loop.
                rubric = caller.call("rubric-repair", rubric_prompt + "\nFix this validation error: " +
                    str(exc) + "\n" + json.dumps(rubric, ensure_ascii=False), [cache / doc["cover_frame"]])
        ...
        transport.atomic_json(cache / "ledger.json", {"source": source, "steps": doc["steps"]})
        metadata = {"model": model, "provider": provider, "profile": profile_name,
            "fallback_reason": fallback_reason, "model_usage": {**asdict(client.usage),
                "call_budget": budget, "cache_hits": caller.cache_hits},
            "transcript_status": speech.status, "warnings": [speech.warning] if speech.warning else []}
        if workspace_mode:
            manifest = publish_workspace(workspace, destination, doc, source, files, metadata,
                                         authoritative_frames, render_html_enabled)
            issues = validate_workspace(workspace)
            if issues:
                raise Error("pipeline tutorial adaptation failed: " + "; ".join(issues))
            return manifest
        if render_html_enabled:
            render_html(destination / files["tutorial"], workspace / "illustrated_tutorial.html")
        return {"schema": SCHEMA, "status": "complete", "package": str(destination),
