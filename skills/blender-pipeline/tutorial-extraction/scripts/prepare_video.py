...
def duration_seconds(ffmpeg, video):
    p = subprocess.run(
        [ffmpeg, "-hide_banner", "-i", str(video)], text=True, capture_output=True
    )
    import re
...
def contact_sheet(frames, dst, cols=5, thumb_width=240):
    ...
    sheet = Image.new(
        "RGB",
        (cols * (size[0] + pad) + pad, rows * (size[1] + label_h + pad) + pad),
        "white",
    )
...
def prepare(video, output, interval=2.0, ocr=False):
    ...
    times = sorted(
        {
            round(min(i * interval, max(0.0, duration - 0.05)), 3)
            for i in range(math.ceil(duration / interval))
        }
        | {round(max(0.0, duration - 0.1), 3)}
    )
    entries = []
    ...
    for start in range(0, len(entries), 24):
        ...
        contact_sheet(
            [output / item["path"] for item in entries[start : start + 24]],
            output / name,
            cols=4,
            thumb_width=360,
        )
    ...
    if ocr:
        ...
        for entry in entries:
            ...
            lines.append(
                f"{entry['timestamp']:.3f}\t{Path(entry['path']).name}\t{' '.join(text.split())}"
            )
